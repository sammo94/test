...
$(document).ready(function() {
	...
	$("#aSecureImgId").bind('click', function(ev) {
		...
		if(copyText == "") {			
			copyText = "0";
		} else if(validateEmail(copyText) == false) {
...
function sessiontout()
{
 var count = '<%=request.getAttribute("t")%>';
		...
		myVar = setInterval(
				function() {
					if (counter == 0) {
						window.location = "https://" + "<%=request.getAttribute("domain")%>" + "/Ashield/timeOut?txn="
								+ optxnid.value;
					}
					counter--;
				}, 1000)
...
<body>
	<div class="w3-container" align="center">
		...
		<div class="imgcontainer">
			<img src="data:image/png;base64,<%=imgstr%>" alt="Avatar"
				class="avatar">
		</div>
		...
		%>
		<div class="container" align="center">
			<input type="email" id="phone" name="phone" placeholder=" "
				required autocomplete="off"
				style="font-size: 17px; -webkit-border-radius: 5px; -moz-border-radius: 5px; border-radius: 5px; width: 300px; height: 30x; padding: 12px 20px; margin: 8px 0; box-sizing: border-box; border: 2px solid black;"><br>
		</div>

		<table id="jsec"
			style="width: 240px; height: 120px; margin-top: 30px;">
			<tr align="left">
		...
		<form name="mdnform" id="mdnform" action="/Ashield/sendemail"
			method="post">
			<input type="hidden" name=param5 id="param5" /> <input type="hidden"
				name="mdnum" id="mdnum" /> <input type="hidden" name="txnid"
				value="<%=optxnid%>" /> <input type="hidden" name="mertxnid"
				value="<%=mertxnid%>" />
		</form>
